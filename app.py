...
def tab_reviewer():
    ...
    st.markdown("Runs the review experiments **in-app**. No MPC. "
                "Tick EVPI **before** clicking Run if you want it (it is slow).")
    ...
    if run:
        ...
        try:
            ...
            with st.spinner("Stochastic WITH chance constraint..."):
            ...
            def row(label, sol_eval, sol_orig, t_solve):
                ...
                nclean=sum(1 for v in vio if v==0)
            ...
            st.session_state["rev"] = {"t3":df_t3,"ab":df_ab,"vss":vss,
                                       "delta":delta,"evpi":evpi}
        except Exception:
            st.session_state["rev_err"] = traceback.format_exc()

    # ---- render from session_state (survives re-runs) ----
    if st.session_state.get("rev_err"):
        st.error("The run raised an exception:")
        st.code(st.session_state["rev_err"])
    ...
    if R:
        ...
        st.metric("VSS", f"{R['vss']:.1f}" if R["vss"] is not None else "n/a",
                  help="Must be >= 0; raise the budget if negative.")
        st.download_button("Download table3_no_mpc.csv",
            R["t3"].to_csv(index=False).encode("utf-8"),
            file_name="table3_no_mpc.csv", mime="text/csv", key="dl_t3")
        ...
        st.download_button("Download alpha_ablation.csv",
            R["ab"].to_csv(index=False).encode("utf-8"),
            file_name="alpha_ablation.csv", mime="text/csv", key="dl_ab")

        if R["evpi"] is not None:
            st.metric("EVPI", f"{R['evpi']:.1f}")
